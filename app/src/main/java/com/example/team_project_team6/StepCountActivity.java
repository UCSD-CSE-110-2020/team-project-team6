...
public class StepCountActivity extends AppCompatActivity {
    ...
    private boolean encouragementShown = false;
...
//       If authentication was required during google fit setup, this will be called after the user authenticates
    ...
    public void setStepCount(long stepCount) {
        ...
        if(!encouragementShown && stepCount >= 1000) {
            showEncouragement(stepCount);
            encouragementShown = true;
        }
    }

    public void showEncouragement(long numSteps) {
        Context context = getApplicationContext();
        int recNumSteps = 100;

        CharSequence text = "Good job! You're already at " + numSteps/recNumSteps + "% of the daily recommended number of steps.";
